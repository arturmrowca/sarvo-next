import { Component, Input, EventEmitter, Output, KeyValueDiffer, KeyValueDiffers } from '@angular/core';
import { IonicPage, NavController, NavParams } from 'ionic-angular';
import { EventDetailsPage } from '../../pages/event-details/event-details';
import { Loadable } from "../../lib/generic/Loadable";
import { SarvoEvent } from "../../lib/SarvoEvent";
import { ViewChildren, QueryList, ViewChild } from '@angular/core';
import { Slides, Scroll } from 'ionic-angular'; 
/**
 * Generated class for the EventListComponent component.
 *
 * See https://angular.io/api/core/Component for more info on Angular
 * Components.
 */
@Component({
  selector: 'event-list',
  templateUrl: 'event-list.html'
})

export class EventListComponent extends Loadable{
  
  @ViewChild('mySlider') mySlider: Slides;
  @ViewChildren('myScroll') components:QueryList<Scroll>;
  @Output() slidesOut = new EventEmitter();

/** private _slideIdx: number;

  emit() {
    this.slidesOut.emit(this._slideIdx)
  }*/

  private _eventsContent: any[] = [];
  private _dateNow: Date;
  private loading: boolean;
  private kvDiffer: KeyValueDiffer<string, any>;
  public static signinCompleted: boolean = false;
  private basicImage:string;

  eventPage = EventDetailsPage;

  constructor(public navCtrl: NavController, public navParams: NavParams, private kvDiffers: KeyValueDiffers) {
    super();
    this.kvDiffer = this.kvDiffers.find({}).create();
    this.loading = false;
    this.basicImage = this.getBasicImage();

  //this._loadEventsContentFromServer(null);
  }

  doRefresh(event){
    this._loadEventsContentFromServer(event);
    
  }

  ngOnInit() {
    if (EventListComponent.signinCompleted)
    {
      this._loadEventsContentFromServer(null);
    }
  }

  // Loads events from server
  _loadEventsContentFromServer(event2:any):void
  {
    this.loading = true;
    this.getDirect("curuser/event/full/").then((requestResponse) => {

      // PARSE RECEIVED ENTRIES
      this._eventsContent = [];

      var eventsFixed = [];
      var eventsPending = [];

      for (let eventDetails of requestResponse as any[])
      {
        let event = new SarvoEvent();
        //console.log("hi -> " + eventDetails["name"]);
        event.name = eventDetails["name"];
        event.description = eventDetails["description"];
        event.fixedDate = eventDetails["fixed_date"];
        event.numMessages = eventDetails["message_count"];
        event.imageBase64 = "";//USE PREVIEW -> see below LOAD THIS SEPARATELY -- eventDetails["imageBase64"];

        event.organizationStatus["DATE"] = eventDetails["organization_status"]["DATE"];
        event.id = eventDetails["id"];

        // load image async
        this.get("event/" + event.id.toString() + "/imagePreview/").then((requestResponse) => {
          event.imagePreviewBase64 = requestResponse["imagePreviewBase64"];
      });

        this._eventsContent.push(event);
        //console.log("Adding to event content " + " "+ this +  this._eventsContent)
      }
      this.loading = false;
      if(event2 != null)
      {
        event2.complete();
      }
        }).catch( (err) => {
          console.log(JSON.stringify(err));
        });
        

  }
  ionViewDidLoad() {
    console.log('ionViewDidLoad B');
    /** this.screenOrientation.onChange().subscribe(() => {
      this.slides.resize();
      this.slides.update();
    });*/
  }

  ngAfterViewInit() { // or ngOnInit or whatever
    setTimeout(() => {
      this._dateNow = this.toLocalDate(new Date());
    });
  }

  ngDoCheck() {
    // Checks if sigin was done
    const changes = this.kvDiffer.diff({"variable": EventListComponent.signinCompleted}); 
    if (changes && EventListComponent.signinCompleted) {
      // Load data
      //console.log("------------------> Expected")
      this._loadEventsContentFromServer(null);
    }
  }

  // extract number of messages of an event to show it as preview.
  getMessages(event): number {
    return event.numMessages;
  }

  toLocalDate(date: Date): Date {
    let dateTemp = new Date(date);
    let localDate = new Date(dateTemp.getTime() - dateTemp.getTimezoneOffset()*60000);

    return localDate;
  }

  // check the status of an event for seqments.
  checkEventStatus(event, eventStatus): boolean {
    if (eventStatus.match("all")) {
      return true;
    } else {
      return event.organizationStatus["DATE"].match(eventStatus);
    }
  }

  // get difference of dates.
  getDateDiff(eventFDate, date: Date) {
    let eventDate: any = new Date(eventFDate);
    let compDate: any = new Date(date);

    // get difference of current date and activity [ms].
    let diff: number = eventDate.getTime() - compDate.getTime();
    // get diff in years.
    let diffYears = eventDate.getUTCFullYear() - compDate.getUTCFullYear();
    // get diff in months.
    let diffMonths = eventDate.getUTCMonth() - compDate.getUTCMonth();
    // get diff in days.
    let diffDayDate = eventDate.getUTCDate() - compDate.getUTCDate();
    // get diff in days.
    let diffDays: number = Math.floor(diff / 86400000);
    // get diff in hours.
    let diffHrs: number = Math.floor((diff % 86400000) / 3600000);
    // get diff in mins.
    let diffMins: number = Math.round(((diff % 86400000) % 3600000) / 60000);

    return {
      diffYears: diffYears,
      diffMonths: diffMonths,
      diffDayDate: diffDayDate,
      diffDays: diffDays,
      diffHrs: diffHrs,
      diffMins: diffMins,
      diffMs: diff
    };
  }

  // check if event date is on the same data as a input date.
  sameDay(event, date: Date): boolean {
    let dateDiff = this.getDateDiff(event.fixedDate, date);

    if ((dateDiff.diffYears + dateDiff.diffMonths + dateDiff.diffDayDate)==0) {
      return true;
    }
    return false;

  }


  // Calculate the difference of the current date and the activity.
  getRemainingTime(event): string {
    if (event.organizationStatus["DATE"].match("pending")) {
      return 'tbd.';
    }
    let diff = this.getDateDiff(event.fixedDate, this._dateNow);

    if (this.sameDay(event, this._dateNow) && ((diff.diffHrs < 0) || (diff.diffMins < 0))) {
      return 'ongoing';
    }

    if (diff.diffDays > 1) {
        return `${ diff.diffDays }d`;
    }
    if (diff.diffDays == 1) {
        return `${ diff.diffDays }d ${ diff.diffHrs }h`;
    }
    if ((diff.diffDays == 0) && (diff.diffHrs > 0)) {
        return `${diff.diffHrs}h ${diff.diffMins}m`;
    }
    if ((diff.diffDays == 0) && (diff.diffHrs == 0)) {
        return `${ diff.diffMins }m`;
    }
  }

  chosen(event: SarvoEvent)
  {
    this.navCtrl.setRoot(this.eventPage, {eventId: event.id});
  }


  getFixedEvents(): Array<SarvoEvent> {
    let fixedEvents: Array<SarvoEvent> = [];
    
    for (let event of this._eventsContent) {
        if (this.checkEventStatus(event, "fixed")) {
          //console.log(event.name);
            let diff = this.getDateDiff(event.fixedDate, this._dateNow);

            if ((this.sameDay(event, this._dateNow)) || (diff.diffMins > 0)) {
                fixedEvents.push(event);
            }
        }
    }
    fixedEvents.sort((e1, e2) => (e1.fixedDate > e2.fixedDate) ? 1 : ((e1.fixedDate < e2.fixedDate) ? -1 : 0));
    return fixedEvents;
  }

  getPendingEvents(): Array<SarvoEvent> {
      let pendingEvents: Array<SarvoEvent> = [];
      //console.log("--> Trying " + " "+ this +  this._eventsContent.length)

      for (let event of this._eventsContent) {
        //console.log(event.name);
          if (this.checkEventStatus(event, "pending")) {
              pendingEvents.push(event);
          }
      }
      return pendingEvents;
  }

  isRefresherEnabled() {
    if (this.mySlider) {
      const activeSlideIdx = this.mySlider.getActiveIndex();
      if (this.components && this.components['_results']) {
        if (this.components['_results'][activeSlideIdx]) {
          if (this.components['_results'][activeSlideIdx]['_scrollContent']) {
            if (this.components['_results'][activeSlideIdx]['_scrollContent'].nativeElement.scrollTop <= 10) {
              return true;
            }
          }
        }
      }
    }
    return false;
  }

  getSliderIdx(): void {
    this.slidesOut.emit(this.mySlider.getActiveIndex())
  }

  getBasicImage()
  {
    return "data:image/jpeg;base64,";
  }
}
